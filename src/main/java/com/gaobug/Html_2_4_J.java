...
/**
 * 面包屑3级,内容全部靠上面 只留h1标签 多模板 描述里面加strong标签 ,单模板
 */
public class Html_2_4_J {
    public static String  setHtml(Map dataMap,String url,String nexturl,String randUrl,String hostName,String html,
        ...
        des=des.replace("<h2>","");
        des=des.replace("</h2>","");
        String strongtag="<strong>"+productname+"</strong>";
        ...
        html=html.replace("{#product_price}",product_price);
            ...
            html=html.replace("{#cate1url}",cate2url);
            html=html.replace("{#cate2}",cate3);
            html=html.replace("{#cate2url}",cate3url);
            html=html.replace("{#cate3}",productname);
            html=html.replace("{#cate3url}",cate4url);
        ...
        int shellcount=Textmysql.ASCIcount(hostName);
            if (products_description.indexOf("。")>0) {
                StringBuffer stringBuilder1=new StringBuffer(products_description);
                int index=stringBuilder1.indexOf("。");
                stringBuilder1.insert(index,strongtag);
                products_description=stringBuilder1.toString();
            }else {
                StringBuffer stringBuilder1=new StringBuffer(products_description);
                stringBuilder1.append(strongtag);
                products_description=stringBuilder1.toString();
            }
...
//                }
        ...
        html=html.replace("{#current_url}",url+hostName);
...
//                if (elements.get(i).attr("href").equals("{#products_url}")){
...
//                }
//                if (elements.get(i).attr("href").equals("{#href#}")){
//
//                }
//            }
