...
public class Html_2_4_3_l {
    public static String  setHtml(Map dataMap,String url,String nexturl,String randUrl,String hostName,String html,
                                 ArrayList listJson,Map readConfigWeb,Map mapId, int idTotal,ArrayList hotkeys,
                                  int randint,ArrayList shellflaglist,int [] discountnum,String secondname){
        try{
        //获取数据
        ...
        String googleImage=dataMap.get("google_image_keyword")!=null?dataMap.get("google_image_keyword").toString():"";
            googleImage=googleImage.replace("Φfenge",",");
            googleImage=googleImage.replace("###","<br>");
        ...
        String ybvideo=dataMap.get("ybvideo")!=null?dataMap.get("ybvideo").toString():"";
       ...
       // html=html.replace("{#cate4}",productname);
        ...
        int urlcount=Textmysql.ASCIcount(url);
        int yhcount=urlcount%10;
        //填充yahoo
        //随机标签
        ...
        if (yahoo_keys!="") {
            ...
            for (int i = 0; i < yhcount; i++) {
                ...
                if (keys.size()>1) {
                   ...
                   String temp1=keys.get(0).replaceAll("<strong>","");
                   String temp2=dess.get(0).replaceAll("<strong>","");
                    html = html.replaceFirst("\\{#h\\d+title\\d+}", Matcher.quoteReplacement(temp1));
                    //html = html.replaceFirst("\\{#googleDes}", "yahoodes"+taghead[shellcount % 5] + Matcher.quoteReplacement(temp2) + tagend[shellcount % 5]);
                    html = html.replaceFirst("\\{#googleDes}", temp2);
                }
            }
            set.clear();
...
//        }
            ...
            html = html.replace("{#youtube}", ybvideo);
...
//                mainimage, price, hostName + "/?" + url+shellflag, hostName, (String) listJson.get(0),cate2url,cate3url,cate4url,cate1,cate2,cate3,"json7");
//        //产品
...
//                mainimage, price, hostName + "/?" + url+shellflag, hostName, (String) listJson.get(0),cate2url,cate3url,cate4url,cate1,cate2,cate3,"json8");
        ...
        alst=Getsitemapurl.getLast(url,mapId,readConfigWeb,urlCount,idTotal);
            ...
            for (int j = 0; j < urls.length-1; j++)
            {
                ...
                //lr模板
                String temp="\"/"+secondname+"?"+url+shellflag+"\""+"title=\""+productname+"\" >"+productname;
                //cc模板
                //String temp="<a href=\"/"+secondname+"?"+url+shellflag+"\""+" title=\""+productname+"\" >"+productname+"</a>";
            ...
            for (int i = 0; i < alst.size(); i++) {
                html=html.replaceFirst("\\{#href#}",hostName+"/"+secondname+"?"+alst.get(i));
            }
