...
/**
 * 多套模板,新模板过滤,面包屑做三级,链接做两种随机
 */



public class Html_2_4_h {
...
//                }
        ...
        alst=Getsitemapurl.getLast(url,mapId,readConfigWeb,urlCount,idTotal);
            ...
            for (int j = 0; j < urls.length-1; j++)
            {
                ...
                int cc=Textmysql.ASCIcount(url);
                String temp="";
                if (cc%2==0) {
                     temp = "\"/?" + url + shellflag + "\"" + "title=\"" + productname + "\" >" + productname;
                }
                else{
                     temp = "\"/?" + url + "\"" + "title=\"" + productname + "\" >" + productname;
                }
            ...
            for (int i = 0; i < alst.size(); i++) {
                html=html.replaceFirst("\\{#href#}",hostName);
            }
